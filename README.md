# 算法文档

## 基础
- [1·算法简介](/docs/base/1·算法简介.md)
- [2·位运算](/docs/base/2·位运算.md)
- [3·排序算法](/docs/base/3·排序算法.md)
- [4·异或运算](/docs/base/4·异或运算.md)

## 数据结构
- [1·数组](/docs/ds/1·数组.md)

## LeetCode
- [1·两数之和](/docs/leetcode/1·两数之和.md)
- [26·删除有序数组中的重复项](/docs/leetcode/26·删除有序数组中的重复项.md)
- [122·买卖股票的最佳时机II](/docs/leetcode/122·买卖股票的最佳时机II.md)

### 剑指offer
- [6·从尾到头打印链表](/docs/leetcode/offer/6·从尾到头打印链表.md)
- [9·用两个栈实现队列](/docs/leetcode/offer/9·用两个栈实现队列.md)
- [24·反转链表](/docs/leetcode/offer/24·反转链表.md)
- [30·包含min函数的栈](/docs/leetcode/offer/30·包含min函数的栈.md)

## 牛客

### 华为
- [1·字符串最后一个单词的长度](/docs/nowcoder/hw/1·字符串最后一个单词的长度.md)
- [2·计算某字符出现次数](/docs/nowcoder/hw/2·计算某字符出现次数.md)
- [3·明明的随机数](/docs/nowcoder/hw/3·明明的随机数.md)
- [17·坐标移动](/docs/nowcoder/hw/17·坐标移动.md)
- [18·识别有效的IP地址和掩码并进行分类统计](/docs/nowcoder/hw/18·识别有效的IP地址和掩码并进行分类统计.md)

## example
- [1·计算阶乘](/docs/example/1·计算阶乘.md)
